...
DhtRunner::shutdown(ShutdownCallback cb, bool stop) {
    ...
    ongoing_ops++;
    shutdownCallbacks_.emplace_back(std::move(cb));
    pending_ops.emplace([=](SecureDhtInterface*) mutable {
...
DhtRunner::getNodeInfo(std::function<void(std::shared_ptr<NodeInfo>)> cb)
{
    std::lock_guard<std::mutex> lck(storage_mtx);
    ongoing_ops++;
    pending_ops_prio.emplace([cb = std::move(cb), this](SecureDhtInterface* dht){
...
DhtRunner::getPublicAddress(std::function<void(std::vector<SockAddr>&&)> cb, sa_family_t af)
{
    std::lock_guard<std::mutex> lck(storage_mtx);
    ongoing_ops++;
    pending_ops_prio.emplace([cb = std::move(cb), this, af](SecureDhtInterface* dht){
...
DhtRunner::get(InfoHash hash, GetCallback vcb, DoneCallback dcb, Value::Filter f, Where w)
{
    ...
    ongoing_ops++;
    pending_ops.emplace([=](SecureDhtInterface* dht) mutable {
...
DhtRunner::cancelListen(InfoHash h, size_t token)
{
    ...
    if (running != State::Running)
        return;
    ongoing_ops++;
    pending_ops.emplace([=](SecureDhtInterface* dht) {
...
DhtRunner::cancelListen(InfoHash h, std::shared_future<size_t> ftoken)
{
    ...
    if (running != State::Running)
        return;
    ongoing_ops++;
    pending_ops.emplace([this, h, ftoken = std::move(ftoken)](SecureDhtInterface* dht) {
...
DhtRunner::put(InfoHash hash, Value&& value, DoneCallback cb, time_point created, bool permanent)
{
    ...
    ] (SecureDhtInterface* dht) mutable {
...
DhtRunner::put(InfoHash hash, std::shared_ptr<Value> value, DoneCallback cb, time_point created, bool permanent)
{
    ...
    ongoing_ops++;
    pending_ops.emplace([=, value = std::move(value), cb = std::move(cb)](SecureDhtInterface* dht) mutable {
...
{
    std::lock_guard<std::mutex> lck(storage_mtx);
    pending_ops_prio.emplace([host, service] (SecureDhtInterface* dht) mutable {
...
{
    std::lock_guard<std::mutex> lck(storage_mtx);
    pending_ops_prio.emplace([host_service = splitPort(hostService)] (SecureDhtInterface* dht) mutable {
...
{
    std::lock_guard<std::mutex> lck(storage_mtx);
    pending_ops_prio.emplace([] (SecureDhtInterface* dht) mutable {
        dht->clearBootstrap();
    });
    cv.notify_all();
}
...
DhtRunner::bootstrap(SockAddr addr, DoneCallbackSimple cb)
{
    ...
    ongoing_ops++;
    pending_ops_prio.emplace([addr = std::move(addr), cb = bindOpDoneCallback(std::move(cb))](SecureDhtInterface* dht) mutable {
...
DhtRunner::bootstrap(const InfoHash& id, const SockAddr& address)
{
    ...
    if (running != State::Running)
        return;
    pending_ops_prio.emplace([id, address](SecureDhtInterface* dht) mutable {
...
DhtRunner::bootstrap(std::vector<NodeExport> nodes)
{
    ...
    if (running != State::Running)
        return;
    pending_ops_prio.emplace([nodes = std::move(nodes)](SecureDhtInterface* dht) {
        for (auto& node : nodes)
            dht->insertNode(node);
    });
    cv.notify_all();
...
{
    std::lock_guard<std::mutex> lck(storage_mtx);
    pending_ops_prio.emplace([=](SecureDhtInterface* dht) {
...
DhtRunner::findCertificate(InfoHash hash, std::function<void(const Sp<crypto::Certificate>&)> cb) {
    ...
    ongoing_ops++;
    pending_ops.emplace([this, hash, cb = std::move(cb)] (SecureDhtInterface* dht) {
    ...
    if (proxify) {
        ...
        auto dht_via_proxy = std::make_unique<DhtProxyClient>(
                ...
                [this]{
                    if (config_.threaded) {
                        std::lock_guard<std::mutex> lck(storage_mtx);
                        pending_ops_prio.emplace([=](SecureDhtInterface*) mutable {});
                        cv.notify_all();
                    }
                },
...
DhtRunner::pushNotificationReceived([[maybe_unused]] const std::map<std::string, std::string>& data)
{
    ...
    std::lock_guard<std::mutex> lck(storage_mtx);
    pending_ops_prio.emplace([ret_token, this, data](SecureDhtInterface*) {
        if (dht_)
